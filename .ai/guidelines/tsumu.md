...
- User、Equipment、Exercise、WorkoutなどDomainデータの主キーと外部キーにはUUIDv7を使う。Laravel内部のjobs、cache、migration管理などは標準の識別子を維持し、内部連番IDと公開UUIDを二重に持たない。UUIDを認可の代替にしない。Domain境界を越えるIDは必要になったものから専用の値オブジェクトにし、共通基底クラスを先回りして作らない。
